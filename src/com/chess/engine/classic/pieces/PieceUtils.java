...
public class PieceUtils {
    ...
    static Pawn getMovedPawn(final Move move) {
        return PieceUtils.ALL_POSSIBLE_PAWNS.get(move.getMovedPiece().getPieceAlliance(), move.getDestinationCoordinate());
    }

    static Knight getMovedKnight(final Move move) {
        return PieceUtils.ALL_POSSIBLE_KNIGHTS.get(move.getMovedPiece().getPieceAlliance(), move.getDestinationCoordinate());
    }

    static Bishop getMovedBishop(final Move move) {
        return PieceUtils.ALL_POSSIBLE_BISHOPS.get(move.getMovedPiece().getPieceAlliance(), move.getDestinationCoordinate());
    }

    static Rook getMovedRook(final Move move) {
        return PieceUtils.ALL_POSSIBLE_ROOKS.get(move.getMovedPiece().getPieceAlliance(), move.getDestinationCoordinate());
    }

    static Queen getMovedQueen(final Move move) {
        return PieceUtils.ALL_POSSIBLE_QUEENS.get(move.getMovedPiece().getPieceAlliance(), move.getDestinationCoordinate());
    }
